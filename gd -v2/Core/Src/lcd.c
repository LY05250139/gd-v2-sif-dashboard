...
void LcdInit(void) 
{
 ...
 //for SYS_EN
  ...
  //for LCD_ON
  P_Strobe(FALSE); 
  LCDSendCom(LCD_CMD, 3);//3位格式命令+
  LCDSendCom(LCD_ON, 8);//写入命令数据
  LCDSendCom(0, 1);
  P_Strobe(TRUE); 
  ...
  P_Strobe(FALSE); 
  LCDSendCom(LCD_CMD, 3);
  LCDSendCom(LCD_BIAS13_COM4, 8);//写入固定值显示特定效果
  LCDSendCom(0, 1);
  P_Strobe(TRUE);
...
void LCDSendCom(uint8_t bVal, uint8_t bCnt)
{
  ...
  for (i = 0; i < bCnt; i++)
  {
    P_Clk(FALSE); //拉低wr脚，data脚在上升沿写入

    P_Data(bVal & 0x80);

    bVal <<= 1; 
    P_Clk(TRUE);//拉高wr脚，为data脚下一次上升沿做准备
  ...
  void LedWrite(uint8_t bTmp)
{
   ...
   void LedWriteRam(uint8_t bAddr,uint8_t bData) 
  {
	...
	P_Strobe(FALSE);
	LedWrite(DATA_SET_INC);  //写入固定值实现特定效果。
	P_Strobe(TRUE);
